...
// Register all tools
server.tool("search_land_use", "Search land use plan (토지이용계획) by address, lot number (지번), road name or building name. Returns zoning (용도지역/지구/구역), building-to-land ratio (건폐율), floor area ratio (용적률), restrictions, and regulatory info. Primary tool for property research.", {
...
server.tool("get_land_details", "Get detailed parcel information including PNU (고유번호), ownership type, current use, appraisal value hints, and full regulatory overlay.", {
    identifier: z.string().describe("PNU code (19 digits) or full address")
}, getLandDetailsTool);
server.tool("search_urban_planning", "Search urban/city planning information (도시계획), district unit plans, development projects by keyword or region.", {
...
server.tool("get_regulation_laws", "Retrieve regulatory law collection (규제법령집) related to land use, zoning restrictions, and relevant ordinances.", {
    keyword: z.string().optional().describe("Optional keyword e.g. '용도지역' or '개발행위허가'")
}, getRegulationLawsTool);
server.tool("search_public_notices", "Search recent public notices (고시정보) for urban planning decisions, land use changes, development projects.", {
    keyword: z.string().optional().describe("Filter by keyword"),
    limit: z.number().int().min(1).max(20).optional().default(5)
}, searchPublicNoticesTool);
server.tool("query_data_open", "Query open datasets (데이터개방) from 토지이음 including development permits, land use regulation CSV/API data. Use dataCd from official list (e.g. '001' for 개발행위허가정보).", {
    dataCd: z.string().describe("Dataset code e.g. '001', '006', '007'"),
    params: z.record(z.any()).optional().describe("Additional query parameters")
}, queryDataOpenTool);
server.tool("verify_land_citations", "Hallucination prevention tool. Extracts land-related legal citations (e.g. '국토의 계획 및 이용에 관한 법률 제36조', '건축법 제11조') from text and verifies against official sources. Returns VALID / INVALID / NOT_FOUND with explanation.", {
...
server.tool("chain_land_analysis", "Comprehensive multi-step land research chain. Analyzes development potential, purchase risks, regulatory compliance, or custom scenarios. Automatically chains relevant tools and provides actionable insights with legal basis.", {
...
server.tool("get_terminology", "Search the official land terminology dictionary (용어사전) for definitions of planning, zoning, and real estate terms.", {
    term: z.string().describe("Term to look up e.g. '용도지역', '건폐율', '지구단위계획'")
}, getTerminologyTool);
server.tool("get_permit_examples", "Retrieve easy-to-understand permit case examples (쉬운 인허가 사례) for common land development and building permit scenarios.", {
    type: z.string().optional().describe("Permit type e.g. '개발행위허가', '건축허가', '용도변경'")
}, getPermitExamplesTool);
// Add discover tool for completeness
server.tool("discover_tools", "List all available tools with descriptions and usage examples. Helps AI discover capabilities dynamically.", {}, async () => ({
    content: [{
            type: "text",
