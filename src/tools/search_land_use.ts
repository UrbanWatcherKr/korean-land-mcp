...
interface LandUseResult {
  ...
  last_updated?: string;
  map_link?: string;
  real_data?: boolean; // 실제 API 사용 여부
}

export const searchLandUseTool = async ({ address, include_map = true }: { address: string; include_map?: boolean }) => {
  ...
  // === 3. 고품질 모의 데이터 (기본값) ===
  const mockResult: LandUseResult = {
    address,
